...
coordinate_dict = {
    ...
    "second_target": np.array([-0.3, 0.2, 0.04]),   
}
...
pick_place_task = CustomPickPlace(
    cube_initial_positions=np.array([[0.3, 0.3, 0.3], [0.4, -0.1, 0.3]]),
        colors=colors
)
...
# Create a mapping from our logical names to the actual cube names in the scene.
cube_keys = {}
for i, cube in enumerate(task_params["cubes"]):
    key = "first_cube" if i == 0 else "second_cube"
...
class PickBehavior(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        # Determine the cube key from the mapping.
        if self.object_param is not None and self.object_param in cube_keys:
            cube_key = cube_keys[self.object_param]
        else:
            cube_key = list(cube_keys.values())[0]

        # Determine pick position from coordinate_dict if available.
        if self.object_param is not None and self.object_param in coordinate_dict:
        ...
        if pick_controller.is_done():
            self.logger.info("[Pick] Pick task completed")
...
class PlaceBehavior(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        # Use the provided location parameter to pick the target position.
        if self.location_param is not None and self.location_param in coordinate_dict:
        ...
        if place_controller.is_done():
            self.logger.info("[Place] Place task completed")
            return py_trees.common.Status.SUCCESS
        return py_trees.common.Status.RUNNING
